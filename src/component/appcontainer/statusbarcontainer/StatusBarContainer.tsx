...
const StatusBarContainer = (statusBarContainerProps: IStatusBarContainer) => {
    ...
    // Trigger UpdateGlobalStatistics API only when featureId changes
    useEffect(() => {
        // Refreshes global app statistics for selected AppQA modules.
        const updateGlobalStats = async () => {
            if (statusBarContainerProps.StatusBartype !== "menu") {
    ...
    useEffect(() => {
        ...
        if (FetchDataError?.length) {
            ...
            if (isSessionTimeout) {
                if (isConfirmOpen)
                    return;
                FnSetSessionStorageItem("user_session", "");
